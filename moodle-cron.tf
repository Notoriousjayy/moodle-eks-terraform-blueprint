resource "kubernetes_cron_job_v1" "moodle_cron" {
  metadata { name = "moodle-cron"; namespace = kubernetes_namespace.moodle.metadata[0].name }
        ...
        template {
          spec {
            ...
            container {
              ...
              env { name = "MOODLE_DATABASE_USER"        value = "app_user" }
              env {
                name = "MOODLE_DATABASE_PASSWORD"
                value_from {
                  secret_key_ref { name = kubernetes_secret.db.metadata[0].name, key = "password" }
                }
              }
